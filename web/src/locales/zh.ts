...
例如：一条带有 1024 维 Embedding 的 1 KB 消息大约使用 9 KB。5 MB 的默认限制大约可容纳 500 条此类消息。`,
    ...
    knowledgeDetails: {
      ...
      retrievalTestingDescription:
        '进行检索测试，检查 RAG 是否能够为大语言模型（LLM）恢复预期的内容。',
      Parse: '解析',
      dataset: '知识库',
      testing: '检索测试',
      configuration: '配置',
      ...
      rerankTip: `非必选项：若不选择 Rerank 模型，系统将默认采用关键词相似度与向量余弦相似度相结合的混合查询方式；如果设置了 Rerank 模型，则混合查询中的向量相似度部分将被 Rerank 打分替代。请注意：采用 Rerank 模型会非常耗时。如需选用 Rerank 模型，建议使用 SaaS 的 Rerank 模型服务；如果你倾向使用本地部署的 Rerank 模型，请务必确保你使用 docker-compose-gpu.yml 启动 RAG。`,
      topK: 'Top-K',
...
示例：general/v2/`,
      ...
      embeddingModelTip:
        '所有新创建的知识库使用的默认 Embedding 模型。如未显示可选模型，请检查你是否在使用 RAG slim 版(不含 Embedding 模型)；或根据 https://ragflow.io/docs/dev/supported_models 确认你的模型供应商是否提供该模型。',
      img2txtModel: 'VLM',
      img2txtModelTip:
    ...
    fileManager: {
      ...
      fileTypeFilter: '文件类型',
      fileTypeFilterTip: '仅可上传所选类型的文件。',
      fileCategoryPdf: 'PDF',
      fileCategoryDocument: '文件与数据',
      fileCategoryAudio: '音频',
      fileCategoryMedia: '图片与视频',
      fileTypeRejected: '已移除 {{name}}：未选中该文件类型',
      uploadingBatch: '正在上传第 {{current}} / {{total}} 批…',
      uploadingFile: '正在上传：{{filename}}',
      uploadSpeed: '{{speed}} kB/s',
      uploadServerProcessing: '服务器处理与写入中，请稍候…',
      filesSelectedSummary: '已选 {{total}} 个文件（显示前 {{shown}} 个）',
      removeAllFiles: '清空全部',
    },
    flow: {
