export default {
  translation: {
    ...
    knowledgeDetails: {
      ...
      uploadDescription: '支持單次或批量上傳。嚴禁上傳違禁檔案。',
      chunk: '解析塊',
      bulk: '批量',
      cancel: '取消',
      rerankModel: 'rerank模型',
      rerankPlaceholder: '請選擇',
      rerankTip: `非必選項：若不選擇 rerank 模型，系統將默認採用關鍵詞相似度與向量餘弦相似度相結合的混合查詢方式；如果設定了 rerank 模型，則混合查詢中的向量相似度部分將被 rerank 打分替代。請注意：採用 rerank 模型會非常耗時。如需選用 rerank 模型，建議使用 SaaS 的 rerank 模型服務；如果你傾向使用本地部署的 rerank 模型，請務必確保你使用 docker-compose-gpu.yml 啟動 RAG。`,
      topK: 'Top-K',
...
以上就是你需要總結的內容。`,
 ...
 NER：使用 spaCy NER 和基於規則的關鍵詞提取來抽取實體和關係，無需 LLM 參與提取過程，速度快且資源消耗低`,
    ...
    setting: {
      ...
      embeddingModel: '嵌入模型',
      embeddingModelTip:
        '如未顯示可選模型，請檢查你是否在使用 RAG slim 版（不含嵌入模型）；或根據 https://ragflow.io/docs/dev/supported_models 確認你的模型供應商是否提供該模型。',
    ...
    fileManager: {
      ...
      fileTypeFilter: '檔案類型',
      fileTypeFilterTip: '僅可上傳所選類型的檔案。',
      fileCategoryPdf: 'PDF',
      fileCategoryDocument: '文件與資料',
      fileCategoryAudio: '音訊',
      fileCategoryMedia: '影像與影片',
      fileTypeRejected: '已移除 {{name}}：未選中該檔案類型',
      uploadingBatch: '正在上傳第 {{current}} / {{total}} 批…',
      uploadingFile: '正在上傳：{{filename}}',
      uploadSpeed: '{{speed}} kB/s',
      uploadServerProcessing: '伺服器處理與寫入中，請稍候…',
      filesSelectedSummary: '已選 {{total}} 個檔案（顯示前 {{shown}} 個）',
      removeAllFiles: '清空全部',
    },
    flow: {
      line: '單行文本',
